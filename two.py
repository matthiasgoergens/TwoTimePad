...
def make_model():
    ...
    conv = lambda base: Maxout(46 * 2 * 2)(
        dropout()(
            Conv1D(filters=46 * 2 * 2 * 5, kernel_size=5, padding="same", strides=1)(
                base
            )
        )
    )
    ...
    lstm = lambda base: (
        Bidirectional(
            LSTM(46 * 2 * 2, dropout=0.1, recurrent_dropout=0.1, return_sequences=True)
        )(base)
    )
    lstm_ed = lstm(conv(lstm(conv(lstm(conv(lstm(conv(embedded))))))))

    totes_clear = Softmax(name="clear")(
        Conv1D(filters=len(alpha), kernel_size=5, padding="same", strides=1)(lstm_ed)
    )
    totes_key = Softmax(name="key")(
        Conv1D(filters=len(alpha), kernel_size=5, padding="same", strides=1)(lstm_ed)
    )
    totes_cipher = Softmax(name="cipher_again")(
        Conv1D(filters=len(alpha), kernel_size=1, padding="same", strides=1)(lstm_ed)
    )
