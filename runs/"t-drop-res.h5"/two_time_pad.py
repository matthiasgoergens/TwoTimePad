...
def make_model_recreate(hparams):
    ...
    totes_clear = Layer(name='clear', dtype='float32')(make_end(SpatialDropout1D(rate=0.05)(concatenate(convedA))))
    totes_key = Layer(name='key', dtype='float32')(make_end(SpatialDropout1D(rate=0.05)(concatenate(convedB))))
