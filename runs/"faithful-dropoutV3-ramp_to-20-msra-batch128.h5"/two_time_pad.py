...
weights_name = "faithful-dropoutV3-ramp_to-20-msra-batch128.h5"
...
def main():
    ...
    with tf.device(device_name):
        ...
        def schedule(epoch):
            default = 0.001
            lr = default * min(20, 1 + epoch * 20 / 4)
        ...
        callbacks_list = [
            ...
            ReduceLROnPlateau(monitor='loss', mode='min', patience=5, cooldown=20, factor=1/2, verbose=1, min_delta=0.001),
...
#           print(f"My epoch: {epoch}")
        if True:
            try:
                model.fit(
                    x=TwoTimePadSequence(l, 10 ** 3 // 16, mtext, both=True),
                    # x = x, y = y,
                    ...
                    validation_data=TwoTimePadSequence(l, 10 ** 2 // 32, mtext, both=True),
                    epochs=100_000,
